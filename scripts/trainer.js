class GlobalManager {
	constructor() {
		...
		this.briefManual = "<h3>四角號碼→漢字検索</h3><ul><li>数値をそのまま入力してください（附角も5桁目の数字として指定します）。</li><li>特定できない桁がある場合、「/」を使って考えられる数値を羅列できます。<br>（例：2か8か分からない場合には「2/8」、5か6か7か分からない場合には「5/6/7」と指定できます。）</li><li>入力中の空白は検索時に除去されます。</li><li>「.」は0〜9のすべてに適合します。</li><li>入力欄左横の「四角號碼」をクリックすると、指定した漢字の四角号碼を調べることができます。</li></ul><br>";
		this.maxItems = 100;
	}
}
